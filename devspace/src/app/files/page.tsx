...
import type { VideoDataClient, Video as DataConnectVideo } from '@/lib/data'; // Using VideoData for page state
...
export default function VideoHubPage() {
  ...
  useEffect(() => {
    ...
    if (DATA_CONNECT_ENDPOINT_CLIENT_CHECK && 
      ...
      const errorMsg = "Firebase Data Connect endpoint is not correctly set. Please include your Connector ID in the NEXT_PUBLIC_DATA_CONNECT_ENDPOINT variable in your .env file and restart the server.";
       ...
       <div className="flex flex-col justify-center items-center h-full text-center p-8">
        ...
        <p className="text-muted-foreground max-w-lg">
         ...
         <p className="text-muted-foreground mt-2 max-w-lg">
          Please check that your <code className="bg-muted px-1 py-0.5 rounded-sm text-sm">.env</code> file has the correct <code className="bg-muted px-1 py-0.5 rounded-sm text-sm">NEXT_PUBLIC_DATA_CONNECT_ENDPOINT</code>, replacing <code className="bg-muted px-1 py-0.5 rounded-sm text-sm">YOUR_CONNECTOR_ID_HERE</code> with your actual Connector ID, and then restart the server.
        </p>
      </div>
    );
  }
